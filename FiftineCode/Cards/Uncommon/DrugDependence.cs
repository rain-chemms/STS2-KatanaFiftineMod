...
//药物依赖每回合获得Chronos相应层数的VigorPower
public class DrugDependence() : FiftineCard(1,
    CardType.Power, CardRarity.Uncommon,
    TargetType.Self)
{
    protected override HashSet<CardTag> CanonicalTags => [CardTag.None];
    protected override IEnumerable<DynamicVar> CanonicalVars => [
        new PowerVar<DrugDependencePower>(1)
    ];

    protected override IEnumerable<IHoverTip> ExtraHoverTips => [
        HoverTipFactory.FromPower<VigorPower>(),
        HoverTipFactory.FromPower<ChronosPower>(),
        HoverTipFactory.FromPower<DrugDependencePower>()
    ];

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        await CommonActions.Apply<DrugDependencePower>(Owner.Creature, this);
    }
